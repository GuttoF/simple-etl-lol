...
async def fetch_and_save_matches(summoner_name: str, region_code: str = "br1"):
    ...
    try:
        ...
        for i, match_id in enumerate(match_ids, 1):
            ...
            try:
                match = await match_manager.get_match_details(match_id, BASE_REGIONS[region_code])
                processed_data = match_manager.process_match_data(match, summoner.puuid)
...
def main():
    ...
    if region_code not in BASE_REGIONS:
        print(f"Invalid region code. Valid options are: {', '.join(BASE_REGIONS.keys())}")
        return
